...
class MySQLStatisticsRepository implements StatisticsRepositoryInterface
{
    ...
    public function getAllStatistics()
    {
        ...
        $stmt_clients = $this->connection->query("SELECT COUNT(*) AS total_clients FROM clients WHERE state = 1");
        $total_clients = $stmt_clients->fetch_assoc()['total_clients'];

        // Total de películas rentadas
        $stmt_rented_movies = $this->connection->query("SELECT COUNT(*) AS total_rented_movies FROM movies WHERE available = false");
        $total_rented_movies = $stmt_rented_movies->fetch_assoc()['total_rented_movies'];

        // Total de películas disponibles
        $stmt_available_movies = $this->connection->query("SELECT COUNT(*) AS total_available_movies FROM movies WHERE available = true");
